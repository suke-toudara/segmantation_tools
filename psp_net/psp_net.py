...
class PSPNet(nn.Module):
    def __init__(self, n_classes,img_width,img_height):
        super(PSPNet, self).__init__()
        # パラメータ設定
        block_config = [3, 4, 6, 3]  # resnet50
        img_width = img_width
        img_width_8 = math.ceil(img_width/8)
        img_height = img_height
        img_height_8 = math.ceil(img_height/8)  # img_sizeの1/8に
        ...
        self.pyramid_pooling = PyramidPooling(in_channels=2048, pool_sizes=[6, 3, 2, 1], height=img_height_8, width=img_width_8)

        self.decode_feature = DecodePSPFeature(
            height=img_height, width=img_width, n_classes=n_classes)

        self.aux = AuxiliaryPSPlayers(
            in_channels=1024, height=img_height, width=img_width, n_classes=n_classes)
